...
dropdown_list= html.Div(id="dropdown",className="dropdown",
            children=[ 
                html.Div(id="dropdown-content",className="hidden",
                    ...
                    children=[
                          dbc.DropdownMenu(label="Main Control Room",
                              children=[dbc.DropdownMenuItem(dcc.Link("InverterRoom1",href="/inverters"))]),])])])
        ...
        dbc.Row([
            ...
            dcc.Graph(id='graph-with-slidersmb2',style={"width":"auto","height":"300px"}, figure={
                            "layout": {
                                "paper_bgcolor": "#272a31",
                                "plot_bgcolor": "#272a31",}}),
            dcc.Link(href='/smb_graph'),],style={'maxHeight':"470px","overflowY":"scroll"})),
...
@app.callback( Output('SEC button', 'value'),[Input('device', 'value'),Input('SEC button', 'n_clicks')],[State('input SEC', 'value')])
def update_output(valueDEV,value2,x):
    print('X=',x)
    if((value2 != None) and (x is not None)):
        client.publish(pubtop,"{} WRITE:SEC_{}".format(valueDEV,x))
        return 'You have published "{} SEC write {}"'.format(valueDEV,x)
#/*------------------------------------------------------------------------------------------------------------------*/

@app.callback( Output('MIN button', 'value'),[Input('device', 'value'),Input('MIN button', 'n_clicks')],[State('input MIN', 'value')])
def update_output(valueDEV,value2,x):
    if((value2 != None) and (x is not None)):
        client.publish(pubtop,"{} WRITE:MIN_{}".format(valueDEV,x))
        return 'You have published "{} MIN write {}"'.format(valueDEV,x)
#/*------------------------------------------------------------------------------------------------------------------*/

@app.callback( Output('HOUR button', 'value'),[Input('device', 'value'),Input('HOUR button', 'n_clicks')],[State('input HOUR', 'value')])
def update_output(valueDEV,value2,x):
    if((value2 != None) and (x is not None)):
        client.publish(pubtop,"{} WRITE:HOUR_{}".format(valueDEV,x))
        return 'You have published "{} HOUR write {}"'.format(valueDEV,x)

#/*------------------------------------------------------------------------------------------------------------------*/

@app.callback( Output('DATE button', 'value'),[Input('device', 'value'),Input('DATE button', 'n_clicks')],[State('input DATE', 'value')])
def update_output(valueDEV,value2,x):
    if((value2 != None) and (x is not None)):
        client.publish(pubtop,"{} WRITE:DATE_{}".format(valueDEV,x))
        return 'You have published "{} DATE write {}"'.format(valueDEV,x)

#/*------------------------------------------------------------------------------------------------------------------*/

@app.callback( Output('MONTH button', 'value'),[Input('device', 'value'),Input('MONTH button', 'n_clicks')],[State('input MONTH', 'value')])
def update_output(valueDEV,value2,x):
    if((value2 != None) and (x is not None)):
        client.publish(pubtop,"{} WRITE:MONTH_{}".format(valueDEV,x))
        return 'You have published "{} MONTH write {}"'.format(valueDEV,x)

#/*------------------------------------------------------------------------------------------------------------------*/

@app.callback( Output('YEAR button', 'value'),[Input('device', 'value'),Input('YEAR button', 'n_clicks')],[State('input YEAR', 'value')])
def update_output(valueDEV,value2,x):
    if((value2 != None) and (x is not None)):
        client.publish(pubtop,"{} WRITE:YEAR_{}".format(valueDEV,x))
        return 'You have published "{} YEAR write {}"'.format(valueDEV,x)

#/*------------------------------------------------------------------------------------------------------------------*/

@app.callback( Output('LAT button', 'value'),[Input('device', 'value'),Input('LAT button', 'n_clicks')],[State('input LAT', 'value')])
def update_output(valueDEV,value2,x):
    if((value2 != None) and (x is not None)):
        client.publish(pubtop,"{} WRITE:LAT_{}".format(valueDEV,x))
        return 'You have published "{} LAT write {}"'.format(valueDEV,x)

#/*------------------------------------------------------------------------------------------------------------------*/

@app.callback( Output('LONGITUDE button', 'value'),[Input('device', 'value'),Input('LONGITUDE button', 'n_clicks')],[State('input LONGITUDE', 'value')])
def update_output(valueDEV,value2,x):
    if((value2 != None) and (x is not None)):
        client.publish(pubtop,"{} WRITE:LONGITUDE_{}".format(valueDEV,x))
        return 'You have published "{} LONGITUDE write {}"'.format(valueDEV,x)


#/*------------------------------------------------------------------------------------------------------------------*/

@app.callback( Output('ELIM button', 'value'),[Input('device', 'value'),Input('ELIM button', 'n_clicks')],[State('input ELIM', 'value')])
def update_output(valueDEV,value2,x):
    if((value2 != None) and (x is not None)):
        client.publish(pubtop,"{} WRITE:ELIM_{}".format(valueDEV,x))
        return 'You have published "{} ELIM write {}"'.format(valueDEV,x)
#/*------------------------------------------------------------------------------------------------------------------*/

@app.callback( Output('WLIM button', 'value'),[Input('device', 'value'),Input('WLIM button', 'n_clicks')],[State('input WLIM', 'value')])
def update_output(valueDEV,value2,x):
    if((value2 != None) and (x is not None)):
        client.publish(pubtop,"{} WRITE:WLIM_{}".format(valueDEV,x))
        return 'You have published "{} WLIM write {}"'.format(valueDEV,x)

#/*------------------------------------------------------------------------------------------------------------------*/

@app.callback( Output('TIMEZONE button', 'value'),[Input('device', 'value'),Input('TIMEZONE button', 'n_clicks')],[State('input TIMEZONE', 'value')])
def update_output(valueDEV,value2,x):
    if((value2 != None) and (x is not None)):
        client.publish(pubtop,"{} WRITE:TIMEZONE_{}".format(valueDEV,x))
        return 'You have published "{} TIMEZONE write {}"'.format(valueDEV,x)
#/*------------------------------------------------------------------------------------------------------------------*/
...
if __name__ == '__main__':
    app.run_server(debug=True,threaded=True, host="0.0.0.0",port=5000,use_reloader=False)
