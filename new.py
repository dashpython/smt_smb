...
app.layout = html.Div(id="big-app-container",
        children=[
            banner,row1,dcc.Location(id='url', refresh=False)],style={  'position':'relative',
'padding':'1rem','display':'flex',"max-width": "100%",'max-height':'100%','flex-direction': 'column','align-items':'center','padding':'0 0.5rem','backgroundColor':'black','height':'140vh','width':'100%'})    
...
def stringtable2():
    ...
    table_body = [html.Tbody([row7, row8, row9, row10, row11, row12, row13])]
    table1 = dbc.Table(
    # using the same table as in the above example
    table_header + table_body,
    bordered=True,
...
IR1 = html.Div([dbc.Container([dbc.Row([dbc.Col([dbc.Card([daq.Indicator(label="DC Switch", color="green", value=True),],style={'width':'12rem','top':'1rem'})]),
    dbc.Col([dbc.Card([daq.Indicator(label="SPD Status", color="green", value=True),],style={'width':'12rem','top':'1rem'})]),]),html.Br(),
    dbc.Row([dbc.Col([dbc.Card([html.H6("DC Voltage"),html.Div("0 V")],style={'width':'12rem','top':'1rem'})]),
    dbc.Col([dbc.Card([html.H6("Sum of Currents"),html.Div("0 A")],style={'width':'12rem','top':'1rem'})])]),html.Br(),
    dbc.Row([dbc.Col([dbc.Card([html.H6("Power"),html.Div("0.000 KW")],)]),])
    ],style={'border':'1px darkblue solid','width':'30rem','margin-top':'2rem'})])

IR2 =html.Div([dbc.Container([dbc.Row([dbc.Col([html.Table(stringtable()),],style={'top':'1rem'}),dbc.Col([html.Table(stringtable2())],style={'top':'1rem'}),])],style={'border':'1px darkblue solid','width':'30rem','margin-top':'2rem','top':'1rem'})])
  

page_16_InverterRoom1 = html.Div([dbc.Row([dbc.Col(IR1),dbc.Col(IR2)]), dcc.Link(href='/page-16')])
